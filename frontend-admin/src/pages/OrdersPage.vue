<template>
  <div class="space-y-4">
        ...
        <table class="w-full">
          ...
          <tbody class="divide-y divide-surface-100 dark:divide-surface-700">
            <tr v-for="order in orders" :key="order._id" class="hover:bg-surface-50 dark:hover:bg-surface-700/50 transition-colors">
              <td class="px-4 py-3 font-mono text-sm text-primary-500">#{{ order._id?.slice(-8).toUpperCase() }}</td>
              <td class="px-4 py-3">
    ...
    <div v-if="deletingOrder" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div class="fixed inset-0 bg-black/50" @click="deletingOrder = null"></div>
