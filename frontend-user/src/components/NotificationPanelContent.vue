  ...
  <div>
    ...
    <div v-else class="divide-y divide-surface-100 dark:divide-surface-700">
      ...
      >
        ...
        <div class="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0"
          ...
          <svg class="w-4 h-4" :class="iconColorClass(notif.type)" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="px-5 py-3 border-t border-surface-200 dark:border-surface-700 bg-surface-50 dark:bg-surface-800/50 flex items-center justify-between">
      ...
      <span v-if="store.notifications.length > 0" class="text-[10px] text-surface-400">
        {{ store.notifications.filter((n: any) => !n.read).length }} unread
      </span>
    </div>
  </div>
</template>
...
function handleClick(notif: any) {
  if (!notif.read) {
    props.store.markAsReadViaSocket(notif._id)
  }
  if (notif.link) {
    emit('close')
