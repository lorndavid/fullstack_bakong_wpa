<template>
  <div class="max-w-4xl mx-auto px-4 sm:px-6 py-4 sm:py-6">
    <h1 class="text-xl sm:text-2xl font-bold text-surface-800 dark:text-white mb-6">My Orders</h1>
    ...
    <div v-if="orders.length === 0" class="text-center py-16">
      ...
      <h2 class="text-lg font-semibold text-surface-700 dark:text-surface-200 mb-2">No orders yet</h2>
      <p class="text-surface-500 mb-6">Start shopping to see your orders</p>
      ...
      <div v-for="order in orders" :key="order._id" class="bg-white dark:bg-surface-800 rounded-2xl p-4 shadow-card hover:shadow-card-hover transition-all cursor-pointer"
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-xs text-surface-500">{{ order.products?.length || 0 }} item(s)</p>
          </div>
...
const orders = ref<any[]>([]) // Will be populated from API
